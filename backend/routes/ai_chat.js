...
const streamChatResponse = async (req, res, requestId, { forceFallback = false } = {}) => {
  ...
  try {
    ...
    const stubPromise = (async () => {
      const stubChunks = [
        '🤖 გურულო პასუხს ამზადებს…',
        '🧠 პასუხის ფორმირება მიმდინარეობს…',
        '⌛ გთხოვ ერთი წამით მოიცადო, პასუხს ვაზუსტებ…',
      ];

      for (let index = 0; index < stubChunks.length; index += 1) {
        if (!sendChunk(stubChunks[index])) {
          return;
        }
        if (index < stubChunks.length - 1) {
          await wait(160);
        }
      }
    })();
