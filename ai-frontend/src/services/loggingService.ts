...
class LoggingService {
  ...
  private async saveLog(logEntry: LogEntry, retryCount = 0): Promise<void> {
    ...
    try {
      ...
      ) as Partial<LogEntry> & Record<string, unknown>;

      // Ensure stackTrace is properly truncated
      if (cleanLogEntry.stackTrace && typeof cleanLogEntry.stackTrace === 'string') {
        cleanLogEntry.stackTrace = this.truncateStackTrace(cleanLogEntry.stackTrace);
      }

      // Add timeout for Firestore operation
      const saveOperation = addDoc(collection(db, 'logs'), {
        ...cleanLogEntry,
        timestamp: Timestamp.fromDate(logEntry.timestamp),
        // Add createdAt if it exists
        ...(logEntry.createdAt && { createdAt: Timestamp.fromDate(logEntry.createdAt) })
      });
