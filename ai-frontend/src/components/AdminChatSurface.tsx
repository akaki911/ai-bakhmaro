...
import type { EmotionalState } from './AiDeveloperPanel';
...
type AdminChatSurfaceProps = {
  children: ReactNode | ((controls: RenderControls) => ReactNode);
  className?: string;
  contentClassName?: string;
};

const AdminChatSurface = ({ children, className, contentClassName }: AdminChatSurfaceProps) => {
  const [emotionalState, setEmotionalState] = useState<EmotionalState>('idle');

  const handleStateChange = useCallback((state: EmotionalState) => {
    setEmotionalState(state);
  }, []);

  const renderedChildren = useMemo(() => {
    if (typeof children === 'function') {
      return (children as (controls: RenderControls) => ReactNode)({
        onEmotionalStateChange: handleStateChange,
      });
    }

    return children;
  }, [children, handleStateChange]);

  const containerClasses = ['relative flex w-full flex-1 min-h-full font-["Inter",_sans-serif] text-slate-100'];
  if (className) {
    containerClasses.push(className);
  }

  const mainClasses = ['relative z-10 flex w-full flex-1 flex-col overflow-hidden px-6 py-8'];
  if (contentClassName) {
    ...
    <div className={containerClasses.join(' ')}>
      ...
      <main className={mainClasses.join(' ')}>
        <div className="relative flex h-full w-full flex-col overflow-hidden rounded-[28px] border border-white/10 bg-slate-900/30 shadow-[0_35px_120px_rgba(6,12,30,0.65)] backdrop-blur-[36px]">
          ...
          <div className="relative z-10 flex h-full flex-col">{renderedChildren}</div>
        </div>
      </main>
    </div>
  );
...
export default AdminChatSurface;
